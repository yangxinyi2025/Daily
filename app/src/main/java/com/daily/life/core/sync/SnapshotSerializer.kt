...
class SnapshotSerializer {
    ...
    private fun encodeSettings(s: SnapshotSettings) = JSONObject().putNullable("semesterStartDate", s.semesterStartDate?.toString()).putNullable("currentSemesterId", s.currentSemesterId).putNullable("targetWeightJin", s.targetWeightJin).putNullable("webDavEndpoint", s.webDavEndpoint).set("autoSyncEnabled", s.autoSyncEnabled)
    private fun decodeSettings(j: JSONObject) = SnapshotSettings(j.optionalString("semesterStartDate")?.let(LocalDate::parse), j.optionalLong("currentSemesterId"), j.optionalDouble("targetWeightJin"), j.optionalString("webDavEndpoint"), j.requiredBoolean("autoSyncEnabled"))
    ...
    private fun encodeScheduleEvent(v: ScheduleEventEntity) = JSONObject().set("id", v.id).set("title", v.title).set("eventAt", v.eventAt).set("reminderOffsetMinutes", v.reminderOffsetMinutes).set("reminderMode", v.reminderMode.name).set("repeatYearly", v.repeatYearly).putNullable("notes", v.notes).putNullable("location", v.location).set("isDismissed", v.isDismissed).set("createdAt", v.createdAt).set("updatedAt", v.updatedAt)
    private fun decodeScheduleEvent(j: JSONObject) = ScheduleEventEntity(j.requiredLong("id"), j.requiredString("title"), j.requiredLong("eventAt"), j.requiredInt("reminderOffsetMinutes"), enumValue(j.requiredString("reminderMode")), j.requiredBoolean("repeatYearly"), j.optionalString("notes"), j.optionalString("location"), j.requiredBoolean("isDismissed"), j.requiredLong("createdAt"), j.requiredLong("updatedAt"))
